...
app=FastAPI(title="Liq-analysis Action GPT Server",version=__version__,description="V4.1 계약에 따라 파일·DB·프로세스·브라우저·Windows UI의 실행-관찰-조작-검증을 제공한다. ",servers=[{"url":PUBLIC_SERVER_URL}] if PUBLIC_SERVER_URL else None,lifespan=lifespan)
...
@app.post("/api/file",operation_id="fileApi",summary="FILE_API",description="STRUCTURE: LIST_ROOT, LIST_DIRECTORY, GET_PATH_INFO. READ: READ_FILE(path), ",response_model=FileApiResponse,dependencies=[Depends(require_key)])
...
@app.post("/api/db",operation_id="dbApi",summary="DB_API",description="기존 DB 연결과 STRUCTURE/READ/EXECUTE/VERIFY 명령 계약을 유지한다.",response_model=ApiResponse,dependencies=[Depends(require_key)])
def db_api(r:DbApiRequest):
    c,cmd=r.category_code.upper(),r.command_code.upper()
    def run():
        target=dict(r.target)
        h={("STRUCTURE","LIST_TABLES"):lambda:db_service.list_tables(target),("STRUCTURE","LIST_COLUMNS"):lambda:db_service.list_columns(target),("READ","READ_DATA"):lambda:db_service.read_data(target,r.options),("READ","EXECUTE_SELECT"):lambda:db_service.execute_select(r.sql,target),("EXECUTE","EXECUTE_DDL"):lambda:db_service.execute_ddl(r.sql,target),("EXECUTE","EXECUTE_DML"):lambda:db_service.execute_dml(r.sql,target),("VERIFY","VERIFY_TABLE"):lambda:db_service.verify_table(target,r.options),("VERIFY","VERIFY_COLUMNS"):lambda:db_service.verify_columns(target,r.options),("VERIFY","VERIFY_DATA"):lambda:db_service.verify_data(r.sql,target,r.options)}
        if (c,cmd) not in h:raise ValueError(f"지원하지 않는 DB_API 명령이다: {c}/{cmd}")
        result=h[(c,cmd)]()
        if isinstance(result,dict):result.setdefault("connection_id",db_service.selected_connection_id(target))
        return ok("DB_API",c,cmd,result,request=r)
    return guarded(run,"DB_API",r)

@app.post("/api/execution",operation_id="executionApi",summary="EXECUTION_API",description="종료형 RUN, 확인한다.",response_model=ApiResponse,dependencies=[Depends(require_key)])
